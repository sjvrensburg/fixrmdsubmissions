...
```{r, eval = FALSE setup}
# [fixrmdsubmissions] Chunk execution disabled due to error: there is no package called ‘nonexistent_package’
# Missing required libraries that will cause errors
library(nonexistent_package)
library(undefined_library)
...
# [fixrmdsubmissions] Chunk execution disabled due to error: '/home/student/Downloads/data.csv' does not exist.
...
another_bad_path <- read.csv(here::here(".", "student_scores.csv"))

# Should use here::here() but doesn't
# [fixrmdsubmissions] Path fixed to use here::here() for portability
yet_another_issue <- readRDS(here::here(".", "model.rds"))

# Missing data file
# [fixrmdsubmissions] Path fixed to use here::here() for portability
missing_file <- read_excel(here::here(".", "nonexistent_file.xlsx"))
```
